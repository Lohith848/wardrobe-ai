  ...
  return (
    <div style={{ minHeight: '100vh', background: '#050505', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '40px 24px' }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <select value={sortBy} onChange={e => setSortBy(e.target.value)}
              style={{ padding: '7px 12px', border: '1px solid #eee', borderRadius: 8,
                fontSize: 13, color: '#a1a1aa', background: '#050505', cursor: 'pointer' }}>
              <option value="newest">Newest first</option>
              <option value="oldest">Oldest first</option>
              <option value="color">By color</option>
            </select>
            <Link href="/upload" style={{ padding: '8px 16px', background: '#ffffff',
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 28 }}>
          {CATS.filter(c => count(c) > 0).map(c => (
            <button key={c} onClick={() => setActiveFilter(c)} style={{
              ...
              borderColor: activeFilter === c ? '#a78bfa' : '#27272a',
              background: activeFilter === c ? '#a78bfa' : '#050505',
              color: activeFilter === c ? '#050505' : '#a1a1aa',
        ...
        {loading && <p style={{ textAlign: 'center', color: '#52525b', padding: 60 }}>Loading...</p>}

        {!loading && filtered.length === 0 && (
          <div style={{ textAlign: 'center', padding: '60px 0' }}>
            <p style={{ fontSize: 40, margin: '0 0 12px' }}>👗</p>
            <p style={{ color: '#52525b', fontSize: 15, margin: '0 0 20px' }}>
              {activeFilter === 'all' ? 'Wardrobe is empty' : `No ${activeFilter}s yet`}
            </p>
            <Link href="/upload" style={{ padding: '11px 22px', background: '#ffffff',
              color: '#050505', borderRadius: 10, textDecoration: 'none',
          ...
          {filtered.map(item => (
            <div key={item.id}
              ...
              style={{ borderRadius: 14, overflow: 'hidden',
                border: '1px solid #f0f0f0', background: '#050505',
                transition: 'all 0.2s' }}>

              {/* Image */}
              <div style={{ height: 200, background: '#161618', position: 'relative' }}>
                {item.image_base64
                ...
                <button onClick={() => deleteItem(item.id)} style={{
                  ...
                  borderRadius: 6, background: 'rgba(0,0,0,0.55)', color: '#050505',
                  border: 'none', cursor: 'pointer', fontSize: 11, fontWeight: 700
                }}>✕</button>
                <div style={{ position: 'absolute', bottom: 8, left: 8,
                  background: 'rgba(0,0,0,0.65)', color: '#050505',
              ...
              <div style={{ padding: '12px 14px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 5 }}>
                  <div style={{ width: 11, height: 11, borderRadius: '50%',
                    background: item.primary_color || '#52525b', border: '1px solid #eee' }} />
                  <span style={{ color: '#a1a1aa', fontSize: 13, textTransform: 'capitalize' }}>
                    {item.primary_color}
                  </span>
                </div>
                <p style={{ color: '#71717a', fontSize: 12, margin: 0, lineHeight: 1.4,
